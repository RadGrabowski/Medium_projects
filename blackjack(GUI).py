...
def load_images(card_images):
    ...
    for suit in suits:
        ...
        for card in range(1, 11):
            name = r'cards_{2}\cards\{0}_{1}.{2}'.format(str(card), suit, extension)
...
def deal_card(frame):
    ...
    # add the image to a label and display that label
...
def score_hand(hand):
    ...
    for next_card in hand:
        ...
        if card_value == 1 and not ace:
            ...
            card_value = 11
        score += card_value
        # if we would bust, check i there is an ace and subtract 10
        if score > 21 and ace:
            score -= 10
            ace = False
    return score
...
result = tk.Label(window, textvariable=result_text).grid(row=0, column=0, columnspan=3)
...
load_images(cards)
print(cards)
# create a new deck of card sand shuffle them
deck = list(cards)
shuffle()
# Create the list to store the dealer's and player's hands
dealer_hand = []
